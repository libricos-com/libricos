 ...
 * @return void
 */
function set_libro( $this2 = null )
{
    if(empty($this2)){
        $this2 = new stdClass();
    }

    $this2->post_type = get_post_type();
    $this2->id = get_the_id();
    $this2->titulo = get_post_meta( $this2->id, 'titulo', true );
    $this2->post_title = get_the_title();
    $this2->asin = get_post_meta( $this2->id, 'asin', true );
    // $this2->asin = $this2->get_product_id();
    
    $this2->url_libro = $this2->url = esc_url( get_permalink( $this2->id ) );

    $this2->pod = pods( 'libro', $this2->id );
    ...
    // Si venimos de los templates de Amazon
    if(!empty($this2->item_index)){
        ...
        $post_terms = wp_get_object_terms( $this2->id, $taxonomy, array( 'fields' => 'ids' ) ); 
